...
locals {
  ...
  iam_role_policy_prefix = "arn:${var.partition}:iam::aws:policy"

  cni_policy = var.cluster_ip_family == "ipv6" ? "arn:${var.partition}:iam::${var.account_id}:policy/AmazonEKS_CNI_IPv6_Policy" : "${local.iam_role_policy_prefix}/AmazonEKS_CNI_Policy"
}
...
data "aws_iam_policy_document" "assume_role_policy" {
  ...
  statement {
    ...
    principals {
      type        = "Service"
      identifiers = ["ec2.${var.partition_dns_suffix}"]
    }
  }
}
...
resource "aws_iam_role_policy_attachment" "this1" {
  policy_arn = aws_iam_policy.eks-xray.arn
  role       = aws_iam_role.this[0].name
}
