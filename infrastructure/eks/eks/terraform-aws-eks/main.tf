...
locals {
  ...
  policy_arn_prefix = "arn:${var.partition}:iam::aws:policy"
  ...
  dns_suffix = coalesce(var.cluster_iam_role_dns_suffix, var.partition_dns_suffix)
}
...
locals {
  ...
  aws_auth_configmap_data = {
    mapRoles = yamlencode(concat(
      ...
      [
        {
          rolearn  = module.crossplane_irsa.arn
          username = "crossplane"
...
#https://aws.amazon.com/ru/blogs/containers/amazon-vpc-cni-increases-pods-per-node-limits/
...
module "enable_prefix" {
  depends_on   = [aws_eks_addon.this]
  source       = "../kubectl"
  cluster_name = aws_eks_cluster.this[0].name
  command      = "set env daemonset aws-node -n kube-system ENABLE_PREFIX_DELEGATION=true"
  region       = var.region
}

module "prefix_warm" {
  source       = "../kubectl"
  depends_on   = [module.enable_prefix]
  cluster_name = aws_eks_cluster.this[0].name
  command      = "set env ds aws-node -n kube-system WARM_PREFIX_TARGET=1"
  region       = var.region
}
