...
resource "aws_security_group" "node" {
  ...
  tags = merge(
    var.tags,
    {
      ...
      "kubernetes.io/cluster/${var.cluster_name}" = "node"
    },
...
module "eks_managed_node_group" {
  ...
  depends_on = [module.prefix_warm, aws_eks_addon.this]
  ...
  #iam_role_additional_policies  = [aws_iam_policy.eks-xray.arn]

  # Security group
  vpc_security_group_ids = compact(concat([
    local.node_security_group_id
  ], try(each.value.vpc_security_group_ids, var.eks_managed_node_group_defaults.vpc_security_group_ids, [])))
...
module "self_managed_node_group" {
  ...
  vpc_security_group_ids = compact(concat([
    local.node_security_group_id
  ], try(each.value.vpc_security_group_ids, var.self_managed_node_group_defaults.vpc_security_group_ids, [])))
  cluster_security_group_id         = local.cluster_security_group_id
