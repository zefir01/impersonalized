...
local regions = {
  default: {
    version:: "v1.11.0-eksbuild.1", // or eg "v1.6.2"
    ecrRegion:: "us-west-2",
...
function(_region){
    local isDefault=!std.member(["us-gov-east-1", "us-gov-west-1", "cn"], _region),
    local region= if isDefault then "default" else _region,
    local node=awsnode+regions[region],
    local helper=metricsHelper+regions[region],

    "items": [i[1] for i in objectItems(node)] + [i[1] for i in objectItems(helper)]

}
#https://raw.githubusercontent.com/aws/amazon-vpc-cni-k8s/v1.11.0/config/master/manifests.jsonnet